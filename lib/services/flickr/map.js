...
function _c(val) { return val && val._content };

exports.contact = {
  id: function(data) {
      return data.nsid;
  },
  name: function(data) { return _c(data.realname) },
  photo: function(data) {
    if(data.iconserver == 0) return null;
    return ["http://farm",data.iconfarm,".staticflickr.com/",data.iconserver,"/buddyicons/",data.nsid,".jpg"].join("");
  },
  oembed: function(data) {
    ...
    if(data.iconserver > 0) ret.thumbnail_url = ["http://farm",data.iconfarm,".staticflickr.com/",data.iconserver,"/buddyicons/",data.nsid,".jpg"].join("");
...
exports.photo = {
  at: function(data) {
    if (data.datetaken) return new Date(data.datetaken).getTime();
    if (data.dateupload) return new Date(parseInt(data.dateupload)*1000).getTime();
    if (data.lastupdate) return new Date(parseInt(data.lastupdate)*1000).getTime();
    return Date.now();
  },
  media: 'source',
  oembed: function(data) {
