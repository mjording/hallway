...
function mapLocation(data) {
  ...
  if (data.venue.categories) {
    ret.categories = [];
    for (var i in data.venue.categories) {
      ret.categories.push({name: data.venue.categories[i].name});
    }
  }
  return ret;
}
...
exports.checkin = {
  ...
  oembed: function(data) {
    ...
    var ret = mapLocation(data);
    ret.type = 'checkin';
    ret.title = data.venue.name; // same as name right now
    ret.url = data.url;
    ret.provider_name = 'foursquare';
    if (data.shout) ret.caption = data.shout;
  ...
  participants: function(data) {
    ...
    if(data.likes && Array.isArray(data.likes.groups)) data.likes.groups.forEach(function(group){
      if(Array.isArray(group.items)) group.items.forEach(function(item){
        if(item.id) ret[item.id] = ret[item.id] || {};
      });
    });
    if(data.comments && Array.isArray(data.comments.items)) data.comments.items.forEach(function(item){
...
exports.photo = {
  ...
  oembed: function(data, idr) {
    var ret = {
      type: 'photo',
      height: data.sizes.items[0].height,
      width: data.sizes.items[0].width,
      url: data.url,
