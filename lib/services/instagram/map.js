...
function mapLocation(data) {
  if (data.location) {
    ...
    var ret = {};
    if(loc.name) ret.name = loc.name;
    if(loc.latitude) ret.lat = loc.latitude;
    if(loc.longitude) ret.lng = loc.longitude;
    return ret;
  }
  return undefined;
}
...
exports.contact = {
  ...
  oembed: function(data) {
    ...
    ret.url = (data.website && data.website.length > 0) ? data.website : 'http://followgram.me/'+data.username;
...
exports.photo = {
  ...
  author: function(data) {
    ...
    ret.url = (data.user.website && data.user.website.length > 0) ? data.user.website : 'http://followgram.me/'+data.user.username;
