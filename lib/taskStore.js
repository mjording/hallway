...
exports.getTasks = function(pid, callback) {
  if(!pid) {
    return process.nextTick(function() {
      callback(new Error('invalid pid:' + pid), {});
    });
  }
  var service = pid.split('@')[1];
  if(!service || !servezas.synclets(service)) {
    return process.nextTick(function() {
      callback('invalid service: ' + service, {});
    });
  }
  var tasks = {};
  var list = isAppGlobalPID(pid)? servezas.globalSyncletList(service) :
               servezas.syncletList(service);
  async.forEach(list, function(synclet, cbLoop) {
    var taskid = 'task:'+service+'/'+synclet+'#'+pid;
    getTask(service, synclet, pid, function(err, task) {
      if (err) return cbLoop(err);
      if (!err && task) tasks[task.idr] = task;
      cbLoop();
    });
  }, function(err) {
    callback(err, tasks);
...
function isAppGlobalPID(pid) {
//app_global_12345678
  if (!pid) return false;
  var id = pid.split('@')[0];
  if (id.indexOf('app_global_') !== 0) return false;
  var appID = id.substring(11);
  if (!appID.match(/[0-9a-f]+/)) return false;
  if (appID.length !== 32) return false;
  return true;
}

exports.createGlobalTasks = function(service, appID, callback) {
  var auth = {
    pid: 'app_global_' + appID + '@' + service
  };

  profileManager.authSet(auth.pid, auth, appID, function(err) {
    if (err) logger.error('error setting auth for global task:' + err);
    var globalSynclets = servezas.globalSynclets(service) || {};
    async.forEach(Object.keys(globalSynclets), function(syncletName, cbLoop) {
      var synclet = globalSynclets[syncletName];
      var taskid = 'task:'+service+'/'+syncletName+'#'+auth.pid;
      exports.saveTask(createTask(taskid, auth.pid, service, syncletName, true), function(err) {
        getTask(service, syncletName, auth.pid, function(err, task) {
          cbLoop();
        });
      });
    }, callback);
...
function getTask(service, synclet, pid, callback) {
  ijod.getOne('task:'+service+'/'+synclet+'#'+pid, callback);
}

exports.getOverdueTasks = function(scount, pid, callback) {
  var pids = {};
  var start = Date.now();
  async.forEachSeries(servezas.serviceList(), function(service, cbSvcLoop) {
    // if this service is already active don't scan for more
    if(scount[service] && scount[service] > lconfig.taskman.scanChunk) {
      return process.nextTick(cbSvcLoop);
    }

    async.forEach(servezas.allSyncletList(service),
      ...
      ijod.getRange('task:' + service + '/' + synclet, {
        until: Date.now(),
        reverse: true,
        limit: lconfig.taskman.scanChunk
      }, function(task) {
        if (pid && task.pid !== pid) return;
    ...
    logger.info("fillNext took", Math.floor((Date.now() - start)/1000),
                plist.length);
    callback(err, plist);
  });
}
