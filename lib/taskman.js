...
function runTask(pi, task, callback)
{
  ...
  function cbDone(err, response) {
    ...
    if (task.count === 0) {
      // if there isn't a config object, return save the task and return
      if (typeof response.config !== 'object') return saveTask(task, callback);
      // otherwise, save the config before returning
      return cleanAndSaveConfig(task, response.config, pi.auth.pid, function() {
    ...
    pipeline.inject(response.data, pi.auth, function(err) {
      ...
      cleanAndSaveConfig(task, response.config, pi.auth.pid);
...
function cleanAndSaveConfig(task, config, pid, callback) {
  ...
  if (config.nextRun) {
    task.nextRun = config.nextRun;
    delete config.nextRun;
  }

  profileManager.configSet(pid, config, callback || function() {});
}
...
exports.fresh = function(base, callback)
{
  ...
  var service = r.host;
  var endpoint = r.path;
  if(!r.host || !SYNCLETS[service]) return callback(new Error("invalid service "+service));
  var pid = [encodeURIComponent(r.auth),r.host].join('@');

  // have to find out which synclets matched
  var matched = {};
  Object.keys(SYNCLETS[service]).forEach(function(sname){
    if(sname == endpoint) matched[sname] = true;
    if(SYNCLETS[service][sname].data.aka && SYNCLETS[service][sname].data.aka.indexOf(endpoint) >= 0) matched[sname] = true;
  });

  // now get all of those tasks and see if they've become stale
  var tasks = {}
  async.forEach(Object.keys(matched), function(synclet, cbLoop) {
    ijod.getOne('task:'+service+'/'+synclet+'#'+pid, function(err, task) {
      if (task && task.at < (Date.now() - STALE_TIME)) tasks[task.idr] = task;
      cbLoop();
    });
  }, function() {
    if(Object.keys(tasks).length == 0) return callback();
