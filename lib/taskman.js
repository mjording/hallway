...
function fillNextScan(scount) {
  taskStore.getOverdueTasks(scount, ONE_PROFILE, function(err, plist) {
    if (plist.length === 0) {
    ...
    async.forEachSeries(plist, function(pid, cbLoop) {
      ...
      if (nexted.length > 0) logger.info("nexted ",nexted.join(" "));
...
exports.taskUpdate = function(auth, callback, force) {
  ...
  if (!servezas.synclets(service)) return callback(new Error("unknown service"));

  // loop through each app
  var apps = (typeof auth.apps === 'object' && Object.keys(auth.apps)) || [];
  taskStore.checkFixedFreq(apps, function(err, fixedfreq) {
